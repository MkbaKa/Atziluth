...
object AttributeUtil {
    ...
    fun registerAttribute(file: File): CustomAttribute {
        ...
        val script = AbstractScriptFactory.compile(file) ?: error("无法根据 ${file.path} 的后缀 ${file.extension} 获取脚本处理器")

        val attributeName = script.getTopLevel<String>("attributeName") ?: error("${file.path} | 无法获取到顶级变量 attributeName")
        ...
        val placeholder = script.getTopLevel<String>("placeholder") ?: error("${file.path} | 无法获取到顶级变量 placeholder")
            ...
            CustomAttribute.buildAttribute(attributeName, attributeType, placeholder) {
                    ...
                    if (script.isFunction(this.attributeType.function)) {
                        when (this.attributeType) {
                            CustomAttributeType.ATTACK, CustomAttributeType.DEFENSE -> this.callback = { attacker, entity, attr, args ->
                                script.invokeFunction(this.attributeType.function, hashMapOf("attacker" to attacker, "entity" to entity, "attr" to attr, "args" to args))
                            }

                            CustomAttributeType.RUNTIME, CustomAttributeType.UPDATE -> this.run = { entity, attr ->
                                script.invokeFunction(this.attributeType.function, hashMapOf("entity" to entity, "attr" to attr))
                            }
                ...
                console().sendLang("register-attribute", Atziluth.prefix, this.attributeName, this.priority, this.combatPower, this.placeholder)
